...
router.post("/group/createTask", Auth, async (req,res)=>{
    ...
    console.log(group)
    const task = { title: req.body.title, description: req.body.description, completion:req.body.completion, type: "group", due: req.body.due, groupid: group._id}
...
router.put("/group/edit", Auth, async(req,res)=>{
   ...
   try {let task = await GroupTask.findOne({"tasks._id": req.body._id});
    ...
    task.tasks[index].description = req.body.description
    task.tasks[index].completion = req.body.completion
    task.tasks[index].due = req.body.due
    task.save()
    res.status(200)
}
    catch(e){
